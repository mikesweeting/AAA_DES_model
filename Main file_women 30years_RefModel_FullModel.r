################################################################################
# SWAN main file, women 30years reference model: full model
################################################################################
require(parallel)
...
# Analysis. 
cat("##### SWAN women 30years reference model: full model", date(), "(runIdentifier=", runIdentifier, ")",sep=" ")
...
if (saveOutput) saveMainAndPsaObjects(processPersonsObjectsToSave, 
			...
			extraText="SWAN_women_30years_reference_model_full_model")
