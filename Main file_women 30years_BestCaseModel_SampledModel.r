################################################################################
# SWAN main file, women 30years best case model: sampled model
################################################################################
require(parallel) 
...
# Parameters.
source("input/parsForWomen30years_best_case_model.R") 
...
# Analysis. 
cat("##### SWAN women 30years best case model: sampled model", date(), "(runIdentifier=", runIdentifier, ")",sep=" ")

systemTimeOutput <- system.time(result <- processPersonsAboveDiagnosisThreshold(v0, v1other, v2, threshold=2.5))
...
if (doPSA) {
  ...
  systemTimeOutput <- system.time(
    psaResultNew <-psaAboveDiagnosisThreshold(v0,v1other,v1distributions,threshold=2.5))
...
if (saveOutput) saveMainAndPsaObjects(processPersonsObjectsToSave, 
			...
			extraText="SWAN_women_30years_best_case_model_sampled_model")
